...
**1. AWSを用いた天気図と気象衛星ひまわり（赤外画像・水蒸気画像）の重ね合わせ画像の配信API**
...
EventBridge ⇒ Lambda(1) ⇒ S3 ⇒ Lambda(2) ⇒ URL
...
（＊）気象庁が公開している衛星画像は、あらかじめWebメルカトルに投影されたタイル画像として配信されているため、それらを縦横必要な枚数取得後に連結し、地上基準点(GCP)により作成した基準天気図（GeoTIFF）の格子へ投影変換して天気図の投影座標系（北緯60度、東経140 度を基準としたポーラーステレオ図法(https://www.data.jma.go.jp/suishin/shiyou/pdf/no12201) に変換し、天気図の表示範囲に対応させて切り取る加工
...
**[QGIS/Tableau/Sentinel-2/オリジナルインデックス]** 林床の含水量ではなく森林の乾燥度から林野火災のリスクを見積もっています。NDWIにかわるオリジナルインデックス(**)を用いています。

(**)NDWIでは冬季の森林の乾燥度の評価ができない（落葉樹林で矛盾した結果がでるため）、バンドB11、B12を用いて木材部分の水分量を評価するインデックスを用いています。

**4. LSTや日射量の計算のための雲マスクの作成**
